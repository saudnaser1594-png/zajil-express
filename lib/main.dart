...
class _AuthScreenState extends State<AuthScreen> {
  ...
  Future<void> _auth(bool isSignUp) async {
    ...
    try {
      if (isSignUp) {
        await supabase.auth.signUp(email: _email.text.trim(), password: _password.text.trim());
        if(mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم انشاء الحساب بنجاح')));
      } else {
              ...
              child: Column(
                ...
                children: [
                  ...
                  _loading ? const CircularProgressIndicator() : Column(children: [
                    ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.red, minimumSize: const Size.fromHeight(45)), onPressed: () => _auth(false), child: const Text('تسجيل الدخول', style: TextStyle(color: Colors.white))),
                    TextButton(onPressed: () => _auth(true), child: const Text('انشاء حساب جديد')),
                  ]),
                ],
              ),
            ),
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if(mounted) {
        ...
        _branch.clear(); _pos.clear(); _required.clear(); _actual.clear(); _total.clear(); _notes.clear();
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [IconButton(icon: const Icon(Icons.logout), onPressed: () async { await supabase.auth.signOut(); if(context.mounted) Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => const AuthScreen())); })],
      ),
              ...
              child: Padding(
                ...
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                      const Text('اضافة سجل جديد', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                      const Divider(height: 24),
                      TextFormField(controller: _branch, decoration: const InputDecoration(labelText: 'اسم الفرع', border: OutlineInputBorder()), validator: (v) => v!.isEmpty ? 'مطلوب' : null),
                      const SizedBox(height: 12),
                      Row(children: [
                        Expanded(child: TextFormField(controller: _pos, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'مبيعات POS', border: OutlineInputBorder()))),
